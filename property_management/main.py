...
def show_rapor_menu(root, show_main_menu_cb):
    ...
    btn_container = tk.Frame(main_frame, bg='#1e1e1e')
    btn_container.pack(expand=True)

    csv_frame = tk.Frame(btn_container, bg='#1e1e1e')
    csv_frame.pack(pady=20)

    tk.Label(csv_frame, text=" CSV RAPORLARI", font=("Arial", 16, "bold"),
             bg='#1e1e1e', fg='white').pack(pady=10)

    csv_buttons = tk.Frame(csv_frame, bg='#1e1e1e')
    csv_buttons.pack()

    create_modern_button(csv_buttons, " Emlak CSV", rapor_emlak, "#4f4f4f", width=18, height=3).pack(side=tk.LEFT, padx=15)
    create_modern_button(csv_buttons, " Müşteri CSV", rapor_musteri, "#5e5e5e", width=18, height=3).pack(side=tk.LEFT, padx=15)
    create_modern_button(csv_buttons, " Komisyon CSV", komisyon_raporu, "#6e6e6e", width=18, height=3).pack(side=tk.LEFT, padx=15)

    other_frame = tk.Frame(btn_container, bg='#1e1e1e')
    other_frame.pack(pady=20)

    tk.Label(other_frame, text=" DİĞER RAPORLAR", font=("Arial", 16, "bold"), bg='#1e1e1e', fg='white').pack(pady=10)

    other_buttons = tk.Frame(other_frame, bg='#1e1e1e')
    other_buttons.pack()

    create_modern_button(other_buttons, " Özet TXT", ozet_rapor, "#6e6e6e", width=18, height=3).pack(side=tk.LEFT, padx=15)
    create_modern_button(other_buttons, " Emlak PDF", lambda: create_pdf_report("emlak"), "#666666", width=18, height=3).pack(side=tk.LEFT, padx=15)
    create_modern_button(other_buttons, " Müşteri PDF", lambda: create_pdf_report("musteri"), "#888888", width=18, height=3).pack(side=tk.LEFT, padx=15)

    create_modern_button(root, " Ana Menü", lambda: show_main_menu_cb(root), "#5c5c5c", width=20, height=3).pack(pady=30)
...
def show_main_menu(root):
    ...
    header_frame = tk.Frame(root, bg='#2c2c2c', height=120)
    header_frame.pack(fill=tk.X, pady=(0, 25))
    header_frame.pack_propagate(False)

    settings = load_settings()

    tk.Label(header_frame, text="", font=("Arial", 30), bg='#2c2c2c', fg='white').pack(pady=(15, 5))
    tk.Label(header_frame, text=settings[1],
             font=("Arial", 20, "bold"), bg='#2c2c2c', fg='white').pack()
    tk.Label(header_frame, text="Professional Edition",
             font=("Arial", 12), bg='#2c2c2c', fg='#BDC3C7').pack(pady=(5, 15))

    menu_container = tk.Frame(root, bg='#1e1e1e')
    menu_container.pack(expand=True)

    tk.Label(menu_container, text="ANA MENÜ", font=("Arial", 18, "bold"),
             bg='#1e1e1e', fg='white').pack(pady=(0, 30))

    row1 = tk.Frame(menu_container, bg='#1e1e1e')
    row1.pack(pady=15)

    create_modern_button(row1, " Emlak\nYönetimi", lambda: show_emlak_menu(root, show_main_menu), "#4f4f4f", width=18, height=5).pack(side=tk.LEFT, padx=20)
    create_modern_button(row1, " Müşteri\nYönetimi", lambda: show_musteri_menu(root, show_main_menu), "#5e5e5e", width=18, height=5).pack(side=tk.LEFT, padx=20)
    create_modern_button(row1, " Randevu\nSistemi", lambda: show_randevu_menu(root, show_main_menu), "#6e6e6e", width=18, height=5).pack(side=tk.LEFT, padx=20)

    row2 = tk.Frame(menu_container, bg='#1e1e1e')
    row2.pack(pady=15)

    create_modern_button(row2, " Komisyon\nHesaplama", lambda: show_komisyon_menu(root, show_main_menu), "#7d7d7d", width=18, height=5).pack(side=tk.LEFT, padx=20)
    create_modern_button(row2, " Dashboard\n& Analiz", lambda: show_dashboard(root), "#6e6e6e", width=18, height=5).pack(side=tk.LEFT, padx=20)
    create_modern_button(row2, " Raporlama\nMerkezi", lambda: show_rapor_menu(root, show_main_menu), "#555555", width=18, height=5).pack(side=tk.LEFT, padx=20)

    row3 = tk.Frame(menu_container, bg='#1e1e1e')
    row3.pack(pady=15)

    create_modern_button(row3, " Email\nSistemi", lambda: show_email_menu(root), "#5e5e5e", width=18, height=5).pack(side=tk.LEFT, padx=20)
    create_modern_button(row3, " Sözleşme\nOluştur", lambda: show_sozlesme_menu(root, show_main_menu), "#666666", width=18, height=5).pack(side=tk.LEFT, padx=20)
    create_modern_button(row3, "️ Sistem\nAyarları", lambda: show_settings(root), "#777777", width=18, height=5).pack(side=tk.LEFT, padx=20)

    row4 = tk.Frame(menu_container, bg='#1e1e1e')
    row4.pack(pady=15)

    create_modern_button(row4, " Takip\nMerkezi", lambda: show_takip_paneli(root), "#6e6e6e", width=18, height=5).pack(side=tk.LEFT, padx=20)

    footer_frame = tk.Frame(root, bg='#1e1e1e')
    footer_frame.pack(side=tk.BOTTOM, pady=20)

    tk.Label(footer_frame, text="═══════════════════════════════════════════════════════════",
             font=("Arial", 10), bg='#1e1e1e', fg='#777777').pack()
    tk.Label(footer_frame, text=" Professional Edition - Emlakçılar için tasarlandı",
             font=("Arial", 11, "bold"), bg='#1e1e1e', fg='#3498DB').pack()
    tk.Label(footer_frame, text=f"© 2026 {settings[1]} • Geliştirici: github.com/skarayil",
             font=("Arial", 9), bg='#1e1e1e', fg='#777777').pack()
